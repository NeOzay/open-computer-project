...
function Electrolyzer.new(side)
	...
	o.side = side
	o.cooldown = 0
	return setmetatable(o, Electrolyzer)
end

---@param dust recipeDust
function Electrolyzer:insertDust(dust)
	transposer.transferItem(drawer.side, self.side, dust.amount, drawer.dustList[dust.name])
...
while true do
	...
	for _, electrolyzer in ipairs(electrolyzerList) do
		...
		if duration == 0 then
			electrolyzer:insertDust(drawer:getDust())
		end
