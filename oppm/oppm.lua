...
local getRepos = cached(
---@return table<string, {repo: string}>|-1|nil
  function()
...
local getPackages = cached(
---@param repo string
---@return table<string, oppm.package>|-1
  function(repo)
    -- Use a "token" parameter to avoid hitting github's cache
    local success, sPackages = pcall(getContent,
      "https://raw.githubusercontent.com/" ..
      repo .. "/master/programs.cfg?token=" .. string.format("%d", math.floor(os.time())))
...
local function listPackages(filter)
  ...
  if not options.i then
    ...
    if lRepos and lRepos.repos then
      for _, j in pairs(lRepos.repos) do ---@cast j table<string, oppm.package>>
...
local function parseFolders(pack, repo, info)
  local function getFolderTable(repo, namePath, branch)
    local success, filestring = pcall(getContent,
      "https://api.github.com/repos/" .. repo .. "/contents/" .. namePath .. "?ref=" .. branch)
  ...
  local function unserializeFiles(files, repo, namePath, branch, relPath)
    ...
    for _, v in pairs(files) do
      if v["type"] == "file" then
        local newPath = v["download_url"]:gsub("https?://raw.githubusercontent.com/" .. nonSpecial(repo) .. "(.+)$", "%1")
        :gsub("/*$", ""):gsub("^/*", "")
        tFiles[newPath] = relPath
      elseif v["type"] == "dir" then
        local newNamePath = namePath .. "/" .. v["name"]
        local newFiles = unserializeFiles(getFolderTable(repo, newNamePath, branch), repo, newNamePath, branch,
          fs.concat(relPath, v["name"]))
        if newFiles then
...
local function installPackage(pack, path, update, tPacks)
  ...
  for i, j in pairs(info.files) do
    ...
    -- Use a "token" parameter to avoid hitting github's cache
    local success, response = pcall(downloadFile,
      "https://raw.githubusercontent.com/" ..
      repo .. "/" .. string.gsub(i, "^%?", "") .. "?token=" .. string.format("%d", math.floor(os.time())), nPath, nil,
      soft)
    if success and response then
      ...
      response .. ". Reverting installation... ")
      fs.remove(nPath)
    ...
    for i, j in pairs(info.dependencies) do
      ...
      if string.lower(string.sub(i, 1, 4)) == "http" then
        ...
        if success and response then
          ...
          pack .. "': " .. response .. ". Reverting installation... ")
          fs.remove(nPath)
          for o, p in pairs(tPacks[pack]) do
            fs.remove(p)
            tPacks[pack][o] = nil
...
local function installToFloppy()
